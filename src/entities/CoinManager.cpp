...
void CoinManager::update(float dt, const sf::FloatRect& playerHitbox, SoundManager* sound) {
    for (auto& coin : mCoins) {
        ...
        sf::FloatRect coinHitbox(coin.position.x - 14.f, coin.position.y + bobY - 14.f, 28.f, 28.f);
...
void CoinManager::draw(sf::RenderWindow& window) {
    ...
    if (mFontLoaded) {
        ...
        for (const auto& ft : mFloatingTexts) {
            ...
            text.setFillColor(sf::Color(255, 230, 40, static_cast<sf::Uint8>(ft.alpha)));
            text.setOutlineColor(sf::Color(20, 15, 0, static_cast<sf::Uint8>(ft.alpha)));
