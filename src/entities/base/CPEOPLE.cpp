...
CPEOPLE::CPEOPLE() {
  ...
  mPosition =
      sf::Vector2f(Win_W / 2.f - Player_W / 2.f, Win_H / 2.f - Player_H / 2.f);

  mSprite.setPosition(mPosition);

  if (mFlashAuraTexture.loadFromFile(
          "assets/shop/animation_shop/flash_speed.png")) {
    mFlashAuraSprite.setTexture(mFlashAuraTexture);
    mFlashAuraSprite.setOrigin(32.f, 32.f);
    mAuraLoaded = true;
  }
  if (mRadarWaveTexture.loadFromFile(
          "assets/shop/animation_shop/radar_wave.png")) {
    mRadarWaveSprite.setTexture(mRadarWaveTexture);
    mRadarWaveSprite.setOrigin(64.f, 64.f);
    mRadarLoaded = true;
  }
  if (mTimeFreezeTexture.loadFromFile(
          "assets/shop/animation_shop/time_freeze.png")) {
...
bool CPEOPLE::loadSprite(const std::string &texturePath) {
  ...
  mAnim =
      std::make_unique<Animation>(mSprite, texture, 64, 64, // frameW, frameH
                                  4, 1,                     // 4 cột, 1 hàng
                                  Frame_Time);
...
void CPEOPLE::reloadSprite(const std::string &texturePath) {
  ...
  const auto &texture = TextureManager::getInstance().getTexture(texturePath);
  mAnim =
      std::make_unique<Animation>(mSprite, texture, 64, 64, 4, 1, Frame_Time);
  mSprite.setScale(Player_W / 64.f, Player_H / 64.f);
  // KO load vị trí mới
}
...
void CPEOPLE::update(float dt) {
  ...
  if (mStats.skillActive) {
    ...
    if (mStats.skillTimer <= 0.f) {
      ...
      mStats.skillCooldownTimer =
          mStats.skillCooldownDuration; // 10.0s cooldown
    }
  } else if (mStats.skillCooldownTimer > 0.f) {
    mStats.skillCooldownTimer -= dt;
    if (mStats.skillCooldownTimer < 0.f)
      mStats.skillCooldownTimer = 0.f;
  }
  ...
  if (mStats.radarActive) {
    ...
    if (mStats.radarTimer <= 0.f) {
      ...
      mStats.radarCooldownTimer =
          mStats.radarCooldownDuration; // 10.0s cooldown
    }
  } else if (mStats.radarCooldownTimer > 0.f) {
    mStats.radarCooldownTimer -= dt;
    if (mStats.radarCooldownTimer < 0.f)
      mStats.radarCooldownTimer = 0.f;
  }
  ...
  if (mStats.timeFreezeActive) {
    ...
    if (mStats.timeFreezeTimer <= 0.f) {
      ...
      mStats.timeFreezeCooldownTimer =
          mStats.timeFreezeCooldownDuration; // 12.0s cooldown
    }
  } else if (mStats.timeFreezeCooldownTimer > 0.f) {
    mStats.timeFreezeCooldownTimer -= dt;
    if (mStats.timeFreezeCooldownTimer < 0.f)
      mStats.timeFreezeCooldownTimer = 0.f;
  }

  if (!mAnim || !mSprite.getTexture())
    return;
...
void CPEOPLE::Move(float dt, int score, bool hasActiveHazard) {
  if (mIsDead || mIsFinish ||
      mControlScheme == PlayerControlScheme::BOT_CONTROLLED)
    return;
  ...
  if (mControlScheme == PlayerControlScheme::PLAYER_2_ARROWS) {
  ...
  } else if (mControlScheme == PlayerControlScheme::PLAYER_1_WASD) {
    // P1: Pure WASD Keys
    ...
    // Single Player: Both WASD and Arrow Keys
    if (sf::Keyboard::isKeyPressed(sf::Keyboard::W) ||
  ...
  // Check Sprinting (Shift keys)
  bool wantSprint = false;
  if (mControlScheme == PlayerControlScheme::PLAYER_2_ARROWS) {
    wantSprint = sf::Keyboard::isKeyPressed(sf::Keyboard::RShift) ||
                 sf::Keyboard::isKeyPressed(sf::Keyboard::RControl) ||
                 sf::Keyboard::isKeyPressed(sf::Keyboard::Numpad0);
  } else if (mControlScheme == PlayerControlScheme::PLAYER_1_WASD) {
    wantSprint = sf::Keyboard::isKeyPressed(sf::Keyboard::LShift);
  } else {
    wantSprint = sf::Keyboard::isKeyPressed(sf::Keyboard::LShift) ||
  ...
  if (mIsMoving && wantSprint && mStats.stamina > 0.f) {
    ...
    if (mStats.stamina < 0.f)
      mStats.stamina = 0.f;
  } else {
    mStats.isSprinting = false;
    mStats.stamina += mStats.staminaRegenRate * dt;
    if (mStats.stamina > mStats.maxStamina)
      mStats.stamina = mStats.maxStamina;
  }

  // Energy drain & regen (Energy drains faster when high score)
  float scoreDrainFactor =
      1.0f + std::min(2.0f, static_cast<float>(score) / 800.f);
  if (mIsMoving) {
    mStats.energy -= mStats.baseEnergyDrain * scoreDrainFactor * dt;
    if (mStats.energy < 0.f)
      mStats.energy = 0.f;
  } else {
  ...
  float totalMultiplier = bootsMultiplier * energyMultiplier * hazardPenalty *
                          (1.0f + skillBoost + sprintBoost) *
                          mPowerUpSpeedMultiplier;
...
bool CPEOPLE::takeDamage(int amount) {
  if (mInvulnerableTimer > 0.f || mIsDead)
    return false;
...
sf::Vector2f CPEOPLE::getPosition() const { return mPosition; }

sf::FloatRect CPEOPLE::getBounds() const { return mSprite.getGlobalBounds(); }

sf::FloatRect CPEOPLE::getHitbox() const {
  sf::FloatRect r = mSprite.getGlobalBounds();
  float dx = r.width * 0.28f;
  float dy = r.height * 0.26f;
  return sf::FloatRect(r.left + dx, r.top + dy, r.width - dx * 2.f,
                       r.height - dy * 2.f);
}

bool CPEOPLE::isDead() const { return mIsDead; }

bool CPEOPLE::isFinish() const { return mIsFinish; }

void CPEOPLE::setDead(bool dead) { mIsDead = dead; }

void CPEOPLE::setFinish(bool finish) { mIsFinish = finish; }

void CPEOPLE::triggerInvulnerability(float duration) {
  mInvulnerableTimer = duration;
}

bool CPEOPLE::isInvulnerable() const { return mInvulnerableTimer > 0.f; }
...
void CPEOPLE::Draw(sf::RenderWindow &window) {
  ...
  if (!mIsBot && mStats.radarActive) {
    float alphaRatio =
        std::clamp(mStats.radarTimer / mStats.radarDuration, 0.f, 1.f);
    ...
    coreRing.setFillColor(
        sf::Color(0, 210, 255, static_cast<sf::Uint8>(45 * alphaRatio)));
    ...
    for (int k = 0; k < 4; ++k) {
      ...
      node.setFillColor(
          sf::Color(255, 235, 100, alpha)); // Golden suction spark
      ...
      sf::Vertex streamLine[] = {
          sf::Vertex(
              sf::Vector2f(px, py),
              sf::Color(0, 240, 255, static_cast<sf::Uint8>(180 * alphaRatio))),
          sf::Vertex(
              sf::Vector2f(playerCenterX, playerCenterY),
              sf::Color(255, 215, 0, static_cast<sf::Uint8>(60 * alphaRatio)))};
  ...
  int slot = (mShopSlot > 0) ? mShopSlot : ShopData::getActiveSlot();
  if (!mIsBot && ShopData::getItemCount("shield", slot) > 0 && !mIsDead) {
  ...
  if (mPowerUpSpeedMultiplier > 1.1f && mIsMoving) {
    ...
    ghost.setPosition(
        mPosition.x - (mRow == 1 ? -10.f : (mRow == 2 ? 10.f : 0.f)),
  ...
  // Player / Bot Identification Badge & 3 Mini Hearts
  if ((mIsBot || !mPlayerLabel.empty()) && !mIsDead && mWarnFontLoaded) {
    std::string badgeStr = mIsBot ? "BOT" : mPlayerLabel;
    sf::Color badgeBgColor =
        mIsBot ? sf::Color(220, 40, 40, 210)
               : (badgeStr == "P1" ? sf::Color(40, 130, 240, 210)
                                   : sf::Color(210, 60, 210, 210));
    sf::Color badgeOutlineColor =
        mIsBot ? sf::Color(255, 220, 220, 240)
               : (badgeStr == "P1" ? sf::Color(180, 220, 255, 240)
                                   : sf::Color(255, 200, 255, 240));

    sf::RectangleShape badgeBg(sf::Vector2f(32.f, 14.f));
    badgeBg.setOrigin(16.f, 7.f);
    badgeBg.setPosition(playerCenterX, mPosition.y - 8.f);
    badgeBg.setFillColor(badgeBgColor);
    badgeBg.setOutlineColor(badgeOutlineColor);
    badgeBg.setOutlineThickness(1.2f);
    window.draw(badgeBg);

    sf::Text badgeText;
    badgeText.setFont(mWarnFont);
    badgeText.setString(badgeStr);
    badgeText.setCharacterSize(11);
    badgeText.setFillColor(sf::Color::White);
    sf::FloatRect b = badgeText.getLocalBounds();
    badgeText.setOrigin(b.left + b.width / 2.f, b.top + b.height / 2.f);
    badgeText.setPosition(playerCenterX, mPosition.y - 8.f);
    window.draw(badgeText);

    // 3 Mini Heart Indicators for HP
  ...
  if (mHasPowerUpShield && !mIsDead) {
    ...
    spark.setPosition(
        playerCenterX + std::cos(sparkAngle) * (bubbleRadius + 3.f),
  ...
  if (mStats.cooldownWarningTimer > 0.f && mWarnFontLoaded) {
    ...
    warnText.setString(mStats.cooldownWarningMsg.empty()
                           ? "SKILL ON COOLDOWN"
                           : mStats.cooldownWarningMsg);
    ...
    float textY =
        mPosition.y - 18.f - (1.2f - mStats.cooldownWarningTimer) * 18.f;
  ...
  if (mStats.stamina < mStats.maxStamina && !mIsDead) {
    ...
    fillBar.setFillColor(mStats.isSprinting ? sf::Color(255, 200, 50, 240)
                                            : sf::Color(50, 220, 100, 240));
    window.draw(fillBar);
  }
}